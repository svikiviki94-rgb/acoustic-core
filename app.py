...
# We'll use 15 mm, 80% porosity, 5 mm cell as the "baseline" (central) configuration
BASE = {
    "t": 15.0,         # mm
    "phi": 0.80,       # porosity fraction
    "d": 5.0           # mm
}

# Interpolate the three datasets to a chosen frequency
...
# Fit local linear coefficients at a frequency:
# A ≈ A_base + k_t*(t-15) + k_p*((1-phi)-(1-0.8)) + k_c*((1/d)-(1/5))
def local_coeffs(freq):
    A10,A15,A20,A90,A80,A70,A3,A5,A7 = interp_at_frequency(freq)

    # thickness slope (least squares over points (10,15,20))
    t_vals = np.array([10,15,20])
    A_t    = np.array([A10,A15,A20])
    kt, bt = np.polyfit(t_vals, A_t, 1)  # A = kt*t + bt
    # convert to delta-from-15mm form -> kt stays as slope

    # porosity slope vs solids fraction s=(1-phi) with phi in [0.7,0.9]
    s_vals = np.array([1-0.90, 1-0.80, 1-0.70])  # [0.10, 0.20, 0.30]
    A_s    = np.array([A90, A80, A70])
    kp, bp = np.polyfit(s_vals, A_s, 1)  # A = kp*s + bp

    # cell size slope vs inv cell size u=1/d for d in [3,5,7.5]
    u_vals = np.array([1/3, 1/5, 1/7.5])
    A_u    = np.array([A3, A5, A7])
    kc, bc = np.polyfit(u_vals, A_u, 1)  # A = kc*u + bc

    # define baseline attenuation (15mm, 80%, 5mm) from the data itself
    A_base = A15  # also equals A80 and A5 in these datasets

    return A_base, kt, kp, kc

# Predict attenuation for arbitrary (t, phi, d) at a frequency using local linearization
...
if len(free_vars) == 0:
    ...
    if sol is None:
        ...
        # Predict attenuation curve across the band using solved parameters
